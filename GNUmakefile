...
download:
	...
	$(call INVOKE_DRIVER,local) download flex nsis fakeroot pkg-config guile	$(foreach p, $(PLATFORMS), (mv uploads/$(p)/lilypond-$(LILYPOND_BRANCH)-1.$(p).gub uploads/$(p)/lilypond-$(LILYPOND_BRANCH)-OLD.$(p).gub || true) &&) true
